                ...
                <el-collapse-item :title="singleData.locale.video" name="video" >
                    <section id="video">
                        <el-skeleton :loading="loading" animated>
                            <div v-if="errorBalancer" class="text-center text-lg font-semibold text-red-500">
                                {{ errorBalancer }}
                            </div>
                            <div v-else-if="!movieData || !movieData.iframeUrl" class="text-center text-lg font-semibold text-gray-500">
                                Movie not found
                            </div>
                            <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4">
<!--                                <div v-if="movieData.poster" class="md:col-span-1">-->
<!--                                    <el-image-->
...
<!--                                </div>-->
                                <div class="md:col-span-2">
                                    <div class="w-full h-[40vh] md:aspect-[16/9] rounded-lg overflow-hidden">
                                        <iframe
                                            :src="movieData.iframeUrl"
                                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; fullscreen"
                                            allowfullscreen
                                            class="w-full h-full"
                                        ></iframe>
                                    </div>
<!--                                    <p v-if="movieData.description" class="mt-4 text-sm md:text-base">-->
...
<script lang="ts" setup>
    ...
    import { ElImage, ElButton, ElSkeleton } from 'element-plus';

    const movieData = ref(null);
    const loading = ref(false);
    const errorBalancer = ref(null);
    ...
    const fetchMovieBalancer = async () => {
        loading.value = true;
        errorBalancer.value = null;
        try {
            const response = await axios.get(`/balancer`, {
                params: { imdb: routeParams.value.id }
            });
            ...
            if (response.data.success) {
                ...
                errorBalancer.value = response.data.error || 'Movie not found';
            }
        } catch (err) {
            console.error('Error loading movie:', err);
            movieData.value = null;
            errorBalancer.value = err.response?.data?.error || 'Server error';
        } finally {
            loading.value = false;
        }
    };
